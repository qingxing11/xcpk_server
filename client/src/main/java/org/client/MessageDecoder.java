...
//		// 读取length
//		ByteBuf lenBuf = in.readBytes(4);
//		byte[] lenData = new byte[lenBuf.readableBytes()];
//		lenBuf.readBytes(lenData);
...
// 		System.err.println("needle.len:"+needle.writerIndex());
		for (int needleIndex = 0; needleIndex < needle.writerIndex(); needleIndex++) {// 取出對比放的一個内容和被對比方進行對比
